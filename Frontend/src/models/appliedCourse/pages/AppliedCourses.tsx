...
export default function AppliedCourses() {
    ...
    var sd = new Date(allAppliedCourses![0].startDate);
    sd.setHours(0, 0, 0, 0);

    console.log(sd)
    console.log(today)
    console.log(sd <= today) // is true
            ...
            <Page>
                <section className="px-4 md:px-24 lg:px-56">
                    ...
                    {allAppliedCourses && allAppliedCourses?.length > 0
                        ? <>
                            {allAppliedCourses && allAppliedCourses.filter(ac => { var sd = new Date(ac.startDate); sd.setHours(0, 0, 0, 0); return sd <= today }).filter(ac => { var ed = new Date(ac.endDate!); ed.setHours(0, 0, 0, 0); return ed >= today }).map((appliedCourse) =>
                                <>
                                    <h1 className="text-xl text-primary mb-2">Active bootcamps</h1>
                            ...
                            {allAppliedCourses && allAppliedCourses.filter(ac => { var sd = new Date(ac.startDate); sd.setHours(0, 0, 0, 0); return sd > today }).map((appliedCourse) =>
                                <>
                                    <h1 className="text-xl mb-2">Future bootcamps</h1>
                                    <div className="collapse border-primary border mb-2">
                                        <input type="checkbox" id={`collapse-toggle-${appliedCourse.id}`} className="hidden" />
                                        <div className="collapse-title flex flex-col w-full gap-4">
                                            <h1 className="text-xl font-bold mb-2 text-primary">{appliedCourse.name} ({new Date(appliedCourse.startDate).getDate()} {monthNames[new Date(appliedCourse.startDate).getMonth()]} {new Date(appliedCourse.startDate).getFullYear()} - {new Date(appliedCourse.endDate!).getDate()} {monthNames[new Date(appliedCourse.endDate!).getMonth()]} {new Date(appliedCourse.endDate!).getFullYear()})</h1>
                            ...
                            {allAppliedCourses && allAppliedCourses.filter(ac => { var ed = new Date(ac.endDate!); ed.setHours(0, 0, 0, 0); return ed < today }).map((appliedCourse) =>
                                <>
                                    <h1 className="text-xl mb-2">Completed bootcamps</h1>
                                    <div className="collapse border-primary border mb-2">
                                        <input type="checkbox" id={`collapse-toggle-${appliedCourse.id}`} className="hidden" />
                                        <div className="collapse-title flex flex-col w-full gap-4">
                                            <h1 className="text-xl font-bold mb-2 text-primary">{appliedCourse.name} ({new Date(appliedCourse.startDate).getDate()} {monthNames[new Date(appliedCourse.startDate).getMonth()]} {new Date(appliedCourse.startDate).getFullYear()} - {new Date(appliedCourse.endDate!).getDate()} {monthNames[new Date(appliedCourse.endDate!).getMonth()]} {new Date(appliedCourse.endDate!).getFullYear()})</h1>
