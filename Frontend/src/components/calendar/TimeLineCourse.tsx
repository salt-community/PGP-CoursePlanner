    ...
    <>
      {dates.map((_currentDate, index) => {
        ...
        <div className=" flex flex-col mb-4 mt-4">
          {index == startIndex
            ? <div style={{ backgroundColor: course.color, "width": widthString}} className="h-10 pl-3 flex items-center font-bold text-white">{course.title} ({format(course.startDate, "d MMM")} - {format(course.endDate, "d MMM")})</div>
            : <>
