...
export const calculateCourseDayDates = (
  ...
  for (let i = 0; i < modules.length; i++) {
    for (let j = 0; j < modules[i].numberOfDays; j++) {
      ...
      calendarDateTypes.push({date: new Date(currentDate), dateContent: [{dayOfModule: modules[i].days[j].dayNumber, totalDaysInModule: modules[i].numberOfDays, courseName: course.name, events: modules[i].days[j].events, color: course.color ? course.color : "#777777", appliedCourseId: course.id, moduleName: modules[i].name }] })
      currentDate.setDate(currentDate.getDate() + 1);
    }
  }
