...
export default function Module({
  ...
  let editTrue = true;
  if (submitFunction.name == "editModule") editTrue = true;
  ...
  const handleDays = () => {
    ...
    if (numOfDays < days.length) {
      ...
      const numOfDaysArray = [...Array(numOfDays - days.length).keys()].map(
        (i) => i + 1
      );
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    const { moduleName } = e.target as typeof e.target & {
      moduleName: { value: string };
    };
    const { numberOfDays } = e.target as typeof e.target & {
    ...
    if (
      isStringInputIncorrect(moduleName.value) ||
            ...
            <div className="relative w-3/5 mr-4">
              {track.length > 0 ? (
                <button
                  ...
                  id="dropdownToggle"
                  onClick={handleClick}
                  className="h-8 text-start pl-3 text-sm text-black w-full border rounded-lg border-gray-300"
                >
                  {track.join(", ")}
                </button>
              ) : (
                <button
                  ...
                  id="dropdownToggle"
                  onClick={handleClick}
                  className="h-8 text-start pl-3 text-sm text-gray-400 w-full border rounded-lg border-gray-300"
                >
                  Select
                </button>
              )}
              ...
              <ul
                ref={dropdownRef}
                id="dropdownMenu"
                className="absolute hidden bg-white py-2 px-2 z-[1000] w-full shadow"
              >
                <li>
