            ...
            {data && data.map((course, index) =>
                ...
                <div key={index} className="flex justify-between items-center bg-white w-full rounded-md drop-shadow-xl">
                    {isLoading ?
                        ...
                        :
                        <>
                            <Link to={`/${bootcamps ? "activecourses" : "courses"}/details/${course.id}`} key={course.name + index} className={`flex justify-between items-center w-full rounded-l-md ${isLoading ? "cursor-default pointer-events-none" : "hover:bg-[#F9F9F9]"}`}>
                            ...
                            <button
                                ...
                                onClick={() => course.id && mutationDelete.mutate(course.id)}>
